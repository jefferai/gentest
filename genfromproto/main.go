...
var (
	...
	inputStructs = []*generateInfo{
		{"inner_proto.go", "InnerProto", "structs/outer_proto.go", "OuterProto", ""},
	}
)
...
func createStructs() {
	for _, inputStruct := range inputStructs {
		...
		ast.Walk(visitFn(func(n ast.Node) {
			...
			if spec.Name.Name != inputStruct.inMsg {
				return
			}

			spec.Name.Name = inputStruct.outMsg
		...
		for scanner.Scan() {
			if !inType {
				if strings.HasPrefix(scanner.Text(), "type "+inputStruct.outMsg) {
					inType = true
				}
			} else {
				if scanner.Text() == "}" {
